...
def make_get_request(reliCost, id, goal):
    response = requests.get('http://pistargodatcc.herokuapp.com/formula/' + reliCost + '?id=BSN_1619832324491&goal=' + goal)
...
def remove_task(data):
    ...
    response = requests.put('http://pistargodatcc.herokuapp.com/formula/' + reliCost + '?id=BSN_1619832324491&goal=G3_T1&shouldPersist=false', json=g3t1)
    formula_tree = response.json()
